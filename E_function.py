#!/usr/bin/python
#
# Andrea Berbellini - INGV-Bologna - September 2019
...
# IN:
#   R file, Z file, period, plot flag (no or yes), event name, station name, catalog folder
#
# OUT:
#   period, ellipticity, s.d., SNR, characteristic function maximum
def E_function(R_file, Z_file, T0, plot, event, station, cmtpath):
    threshold = 0.6         # pre-defined threhold (abs(ccf)*envelope)
    seconds_of_noise = 2400 # noise time window from begin time
    k = 1.4                 # period/frequncy range
    Tmin = T0 / np.sqrt(k)
    Tmax = T0 * np.sqrt(k)
    Fmin = 1/Tmax
    Fmax = 1/Tmin
    ...
    # not used, commented
    #ell_sum  = 0.0      # sum of ellipticity
    #num_data = 0.0      # num of data
    #av_ell   = 0.0      # average of ellpticity
    #sd_total = 0.0      # total s.d
    ...
    #   start-100 ~ end+100 of Z component
    #-----------------------------------------------
    ...
    if found == 'yes':
        ...
        Z_R_cut = []    # cutted HV envelope ratio
        E  = 0.0        # mean ellipticity
        sd = 0.0        # s.d.
        ...
        E  = Z_R_cut_mean
        sd = Z_R_cut_sd
        #sd_perc = 100 * sd / E     # not used, commented
